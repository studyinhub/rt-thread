 ...
 * 2010-04-01     Bernard      add prompt output when start and remove the empty
 * history 2011-02-23     Bernard      fix variable section end issue of finsh
 * shell initialization when use GNU GCC compiler. 2016-11-26     armink add
 * password authentication 2018-07-02     aozima       add custom prompt
 * support.
 */
...
struct finsh_syscall *finsh_syscall_next(struct finsh_syscall *call) {
  ...
  while ((*ptr == 0) &&
         ((unsigned int *)ptr < (unsigned int *)_syscall_table_end))
    ptr++;
...
struct finsh_sysvar *finsh_sysvar_next(struct finsh_sysvar *call) {
  ...
  ptr = (unsigned int *)(call + 1);
  while ((*ptr == 0) &&
         ((unsigned int *)ptr < (unsigned int *)_sysvar_table_end))
    ptr++;
...
#define _PROMPT "finsh "
  ...
  //   getcwd(&finsh_prompt[rt_strlen(finsh_prompt)],
  //          RT_CONSOLEBUF_SIZE - rt_strlen(finsh_prompt));
  // #endif
 ...
 *
 * @return prompt the prompt mode, 0 disable prompt mode, other values enable
 * prompt mode.
 */
 ...
 *
 * @return result, RT_EOK on OK, -RT_ERROR on the new password length is less
 * than FINSH_PASSWORD_MIN or greater than FINSH_PASSWORD_MAX
 */
...
static void shell_push_history(struct finsh_shell *shell) {
  if (shell->line_position != 0) {
    ...
    if (shell->history_count >= FINSH_HISTORY_LINES) {
      ...
      if (memcmp(&shell->cmd_history[FINSH_HISTORY_LINES - 1], shell->line,
                 FINSH_CMD_SIZE)) {
        /* move history */
        int index;
...
__declspec(allocate("FSymTab$a")) const struct finsh_syscall __fsym_begin = {
    __fsym_begin_name, __fsym_begin_desc, NULL};
...
__declspec(allocate("FSymTab$z")) const struct finsh_syscall __fsym_end = {
    __fsym_end_name, __fsym_end_desc, NULL};
#endif
...
int finsh_system_init(void) {
  ...
  tid = rt_thread_create(FINSH_THREAD_NAME, finsh_thread_entry, RT_NULL,
                         FINSH_THREAD_STACK_SIZE, FINSH_THREAD_PRIORITY, 10);
#else
  shell = &_shell;
  tid = &finsh_thread;
  result =
      rt_thread_init(&finsh_thread, FINSH_THREAD_NAME, finsh_thread_entry,
                     RT_NULL, &finsh_thread_stack[0],
                     sizeof(finsh_thread_stack), FINSH_THREAD_PRIORITY, 10);
#endif /* RT_USING_HEAP */
