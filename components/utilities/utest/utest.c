...
static void utest_tc_list(void) {
  ...
  for (i = 0; i < tc_num; i++) {
    LOG_I("[testcase name]:%s; [run timeout]:%d", tc_table[i].name,
...
static int utest_help(void) {
  ...
  rt_kprintf("            support '*' wildcard. Run all test cases starting "
             "with 'test'.\n");
...
static void utest_testcase_run(int argc, char **argv) {
  ...
  if (argc == 1) {
    ...
    if (rt_strcmp(argv[1], "-thread") == 0) {
      ...
      tid =
          rt_thread_create("utest", (void (*)(void *))utest_run, thr_param,
...
MSH_CMD_EXPORT_ALIAS(utest_testcase_run, utest_run,
                     utest_run[-thread or -help][testcase name][loop num]);

utest_t utest_handle_get(void) { return (utest_t)&local_utest; }
...
void utest_assert(int value, const char *file, int line, const char *func,
                  const char *msg) {
  if (!(value)) {
