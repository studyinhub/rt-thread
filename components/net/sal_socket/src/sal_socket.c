...
#define IS_SOCKET_PROTO_TLS(sock)                                              \
  (((sock)->protocol == PROTOCOL_TLS) || ((sock)->protocol == PROTOCOL_DTLS))
#define SAL_SOCKOPS_PROTO_TLS_VALID(sock, name)                                \
  (proto_tls && (proto_tls->ops->name) && IS_SOCKET_PROTO_TLS(sock))
...
int sal_init(void) {
  ...
  cn = SOCKET_TABLE_STEP_LEN < SAL_SOCKETS_NUM ? SOCKET_TABLE_STEP_LEN
                                               : SAL_SOCKETS_NUM;
...
static void check_netdev_internet_up_work(struct rt_work *work,
                                          void *work_data) {
...
#define SAL_INTERNET_DATE_LEN 16
  ...
  const char month[][SAL_INTERNET_MONTH_LEN] = {"Jan", "Feb", "Mar", "Apr",
                                                "May", "Jun", "Jul", "Aug",
                                                "Sep", "Oct", "Nov", "Dec"};
  ...
  struct sal_proto_family *pf =
      (struct sal_proto_family *)netdev->sal_user_data;
  ...
  skt_ops->setsockopt(sockfd, SOL_SOCKET, SO_RCVTIMEO, (void *)&timeout,
                      sizeof(timeout));
  skt_ops->setsockopt(sockfd, SOL_SOCKET, SO_SNDTIMEO, (void *)&timeout,
                      sizeof(timeout));
  ...
  result = skt_ops->recvfrom(sockfd, &recv_data, sizeof(recv_data), 0,
 ...
 * @note please don't invoke it on ISR.
 */
static void sal_unlock(void) { rt_mutex_release(&sal_core_lock); }
...
int sal_netdev_cleanup(struct netdev *netdev) {
  ...
  do {
    ...
    for (idx = 0; idx < socket_table.max_socket; idx++) {
      if (socket_table.sockets[idx] &&
          socket_table.sockets[idx]->netdev == netdev) {
        find_dev = 1;
        break;
      }
    }
 ...
 */
static int socket_init(int family, int type, int protocol,
                       struct sal_socket **res) {
  ...
  if (netdv_def && netdev_is_up(netdv_def)) {
    ...
    if (pf != RT_NULL && pf->skt_ops &&
        (pf->family == family || pf->sec_family == family)) {
  ...
  if (flag == RT_FALSE) {
    ...
    if (netdev == RT_NULL) {
      LOG_E("not find network interface device by protocol family(%d).",
            family);
      return -3;
    }
...
static void sal_sockaddr_to_ipaddr(const struct sockaddr *name,
...
int sal_bind(int socket, const struct sockaddr *name, socklen_t namelen) {
  ...
  if (!ip_addr_isany_val(input_ipaddr)) {
    ...
    if (input_pf->family != local_pf->family) {
      ...
      /* protocol family is different, close old socket and create new socket by
       * input ip address */
      local_pf->skt_ops->closesocket(socket);

      new_socket = input_pf->skt_ops->socket(input_pf->family, sock->type,
                                             sock->protocol);
      if (new_socket < 0) {
        return -1;
      }
...
int sal_getsockopt(int socket, int level, int optname, void *optval,
                   socklen_t *optlen) {
  struct sal_socket *sock;
  struct sal_proto_family *pf;
  ...
  return pf->skt_ops->getsockopt((int)sock->user_data, level, optname, optval,
                                 optlen);
}

int sal_setsockopt(int socket, int level, int optname, const void *optval,
                   socklen_t optlen) {
  struct sal_socket *sock;
  struct sal_proto_family *pf;
  ...
  if (level == SOL_TLS) {
    ...
    return pf->skt_ops->setsockopt((int)sock->user_data, level, optname, optval,
                                   optlen);
  }
#else
  return pf->skt_ops->setsockopt((int)sock->user_data, level, optname, optval,
...
int sal_sendto(int socket, const void *dataptr, size_t size, int flags,
  ...
  if (SAL_SOCKOPS_PROTO_TLS_VALID(sock, send)) {
    ...
    return pf->skt_ops->sendto((int)sock->user_data, dataptr, size, flags, to,
                               tolen);
  }
#else
  return pf->skt_ops->sendto((int)sock->user_data, dataptr, size, flags, to,
...
int sal_gethostbyname_r(const char *name, struct hostent *ret, char *buf,
  ...
  if (SAL_NETDEV_NETDBOPS_VALID(netdev, pf, gethostbyname_r)) {
    return pf->netdb_ops->gethostbyname_r(name, ret, buf, buflen, result,
                                          h_errnop);
  } else {
    /* get the first network interface device with up status */
    netdev = netdev_get_first_by_flags(NETDEV_FLAG_UP);
