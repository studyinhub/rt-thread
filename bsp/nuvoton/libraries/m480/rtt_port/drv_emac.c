...
static rt_err_t nu_emac_init(rt_device_t dev) {
  ...
  result =
      rt_thread_init(&eth_tid, "eth", link_monitor, (void *)psNuEMAC, eth_stack,
                     sizeof(eth_stack), RT_THREAD_PRIORITY_MAX - 2, 10);
  RT_ASSERT(result == RT_EOK);
...
static int rt_hw_nu_emac_init(void) { return rt_hw_nu_emac_register("e0"); }
