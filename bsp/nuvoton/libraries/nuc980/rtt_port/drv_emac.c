...
static err_t nu_igmp_mac_filter(struct netif *netif, const ip4_addr_t *ip4_addr,
...
static rt_size_t nu_emac_read(rt_device_t dev, rt_off_t pos, void *buffer,
                              rt_size_t size);
static rt_size_t nu_emac_write(rt_device_t dev, rt_off_t pos,
...
static void nu_emac_reinit(nu_emac_t psNuEmac) {
  ...
  for (rt_uint8_t index = 0; index < EMAC_CAMENTRY_NB; index++) {
    rt_uint32_t *CAMxM =
        (rt_uint32_t *)((rt_uint32_t)&EMAC->CAM0M + (index * 8));
    rt_uint32_t *CAMxL =
        (rt_uint32_t *)((rt_uint32_t)&EMAC->CAM0L + (index * 8));
...
static err_t nu_igmp_mac_filter(struct netif *netif, const ip4_addr_t *ip4_addr,
  ...
  if (ret >= 0) {
    NU_EMAC_TRACE("%s %s %s ", __FUNCTION__,
                  (action == NETIF_ADD_MAC_FILTER) ? "add" : "del",
                  ip4addr_ntoa(ip4_addr));
...
static void nu_memmgr_init(EMAC_MEMMGR_T *psMemMgr) {
  ...
  psMemMgr->psTXDescs = (EMAC_DESCRIPTOR_T *)rt_malloc_align(
      sizeof(EMAC_DESCRIPTOR_T) * psMemMgr->u32TxDescSize, 32);
  RT_ASSERT(psMemMgr->psTXDescs != RT_NULL);

  psMemMgr->psRXDescs = (EMAC_DESCRIPTOR_T *)rt_malloc_align(
      sizeof(EMAC_DESCRIPTOR_T) * psMemMgr->u32RxDescSize, 32);
  RT_ASSERT(psMemMgr->psRXDescs != RT_NULL);

  psMemMgr->psTXFrames = (EMAC_FRAME_T *)rt_malloc_align(
      sizeof(EMAC_FRAME_T) * psMemMgr->u32TxDescSize, 32);
  RT_ASSERT(psMemMgr->psTXFrames != RT_NULL);

  psMemMgr->psRXFrames = (EMAC_FRAME_T *)rt_malloc_align(
      sizeof(EMAC_FRAME_T) * psMemMgr->u32RxDescSize, 32);
...
static rt_err_t nu_emac_init(rt_device_t dev) {
  ...
  mmu_clean_invalidated_dcache((uint32_t)psNuEmac->memmgr.psTXDescs,
                               sizeof(EMAC_DESCRIPTOR_T) *
                                   psNuEmac->memmgr.u32TxDescSize);
  mmu_clean_invalidated_dcache((uint32_t)psNuEmac->memmgr.psRXDescs,
                               sizeof(EMAC_DESCRIPTOR_T) *
                                   psNuEmac->memmgr.u32RxDescSize);
#endif
  ...
  psNuEmac->link_monitor =
      rt_thread_create((const char *)szTmp, link_monitor, (void *)psNuEmac,
                       NU_EMAC_TID_STACK_SIZE, RT_THREAD_PRIORITY_MAX - 2, 10);
...
static rt_err_t nu_emac_close(rt_device_t dev) { return RT_EOK; }
...
static rt_err_t nu_emac_tx(rt_device_t dev, struct pbuf *p) {
  ...
  /* Return SUCCESS? */
#if defined(BSP_USING_MMU)
  mmu_clean_invalidated_dcache((uint32_t)psNuEmac->memmgr.psCurrentTxDesc,
                               sizeof(EMAC_DESCRIPTOR_T));
#endif
  return (EMAC_SendPktWoCopy(&psNuEmac->memmgr, offset) == 1) ? RT_EOK
                                                              : RT_ERROR;
}
...
static struct pbuf *nu_emac_rx(rt_device_t dev) {
...
#if defined(BSP_USING_MMU)
  mmu_clean_invalidated_dcache((uint32_t)psNuEmac->memmgr.psCurrentRxDesc,
                               sizeof(EMAC_DESCRIPTOR_T));
#endif
  if ((avaialbe_size = EMAC_GetAvailRXBufSize(&psNuEmac->memmgr, &pu8DataBuf)) >
      0) {
  ...
  } else /* If it hasn't RX packet, it will enable interrupt. */
  {
    ...
    if (!(EMAC->INTEN & EMAC_INTEN_RDUIEN_Msk)) {
      EMAC_CLEAR_INT_FLAG(EMAC,
                          (EMAC_INTSTS_RDUIF_Msk | EMAC_INTSTS_RXGDIF_Msk));
...
static void rt_hw_nu_emac_assign_macaddr(nu_emac_t psNuEMAC) {
  ...
  //               psNuEMAC->mac_addr[0], psNuEMAC->mac_addr[1],
  //               psNuEMAC->mac_addr[2], psNuEMAC->mac_addr[3],
  //               psNuEMAC->mac_addr[4], psNuEMAC->mac_addr[5]);
  value++;
}
...
static int rt_hw_nu_emac_init(void) {
  ...
  for (i = (EMAC_START + 1); i < EMAC_CNT; i++) {
    ...
    rt_hw_interrupt_install(psNuEMAC->irqn_tx, nu_emac_tx_isr, (void *)psNuEMAC,
                            szTmp);
    rt_hw_interrupt_umask(psNuEMAC->irqn_tx);

    snprintf(szTmp, sizeof(szTmp), "%s_rx", psNuEMAC->name);
    rt_hw_interrupt_install(psNuEMAC->irqn_rx, nu_emac_rx_isr, (void *)psNuEMAC,
                            szTmp);
