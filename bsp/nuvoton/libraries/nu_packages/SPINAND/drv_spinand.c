...
rt_size_t nu_qspi_transfer_message(struct rt_qspi_device *device,
...
rt_err_t nu_qspi_send_then_recv(struct rt_qspi_device *device,
...
rt_err_t nu_qspi_send(struct rt_qspi_device *device, const void *send_buf,
                      rt_size_t length) {
  RT_ASSERT(send_buf);
  RT_ASSERT(length != 0);
...
static rt_err_t spinand_read_page(struct rt_mtd_nand_device *device,
  ...
  // LOG_D("[R-%d]data: 0x%08x %d, spare: 0x%08x, %d", page, data, data_len,
  // spare, spare_len);
  ...
  result =
      SPINAND_FLASH_OPS->read_dataload(SPINAND_FLASH_QSPI, (page >> 16) & 0xFF,
  ...
  if (data && data_len) {
    ...
    result = SPINAND_FLASH_OPS->read_quadoutput(SPINAND_FLASH_QSPI, 0, 0, data,
                                                data_len);
    if (result != RT_EOK)
      goto exit_spinand_read_page;
  }
  ...
  if (spare && spare_len) {
    ...
    result = SPINAND_FLASH_OPS->read_quadoutput(
        SPINAND_FLASH_QSPI, (SPINAND_FLASH_PAGE_SIZE >> 8) & 0xff,
        SPINAND_FLASH_PAGE_SIZE & 0xff, spare, spare_len);
    if (result != RT_EOK)
      goto exit_spinand_read_page;
  }
...
static rt_err_t spinand_write_page(struct rt_mtd_nand_device *device,
  ...
  LOG_D("[W-%d]data: 0x%08x %d, spare: 0x%08x, %d", page, data, data_len, spare,
        spare_len);
  ...
  if (SPINAND_FLASH_MCP == 1) {
    /* Select die. */
    if ((result = SPINAND_FLASH_OPS->die_select(SPINAND_FLASH_QSPI,
                                                SPINAND_DIE_ID0)) != RT_EOK)
      goto exit_spinand_write_page;
  }

  /* Read data: 0~2111, to cache */
  if (data && data_len)
    result = SPINAND_FLASH_OPS->program_dataload(SPINAND_FLASH_QSPI, 0, 0,
                                                 (uint8_t *)data, data_len,
                                                 (uint8_t *)spare, spare_len);
  else
    result = SPINAND_FLASH_OPS->program_dataload(
        SPINAND_FLASH_QSPI, (SPINAND_FLASH_PAGE_SIZE >> 8) & 0xff,
        SPINAND_FLASH_PAGE_SIZE & 0xff, RT_NULL, 0, (uint8_t *)spare,
        spare_len);
  ...
  result = SPINAND_FLASH_OPS->program_execute(
      SPINAND_FLASH_QSPI, (((page) >> 16) & 0xFF), (((page) >> 8) & 0xFF),
      (page) & 0xFF);
  if (result != RT_EOK)
    goto exit_spinand_write_page;
...
static rt_err_t spinand_move_page(struct rt_mtd_nand_device *device,
  ...
  /* Data load, Read data from flash to cache */
  result = SPINAND_FLASH_OPS->read_dataload(
      SPINAND_FLASH_QSPI, (src_page >> 16) & 0xFF, (src_page >> 8) & 0xFF,
      (src_page & 0xFF));
  if (result != RT_EOK)
  ...
  u8WECmd = 0x06;
  if ((result = nu_qspi_send(SPINAND_FLASH_QSPI, &u8WECmd, sizeof(u8WECmd))) !=
      RT_EOK)
    goto exit_spinand_move_page;

  /* Flush cache to flash */
  result = SPINAND_FLASH_OPS->program_execute(
      SPINAND_FLASH_QSPI, (((dst_page) >> 16) & 0xFF),
      (((dst_page) >> 8) & 0xFF), (dst_page) & 0xFF);
  if (result != RT_EOK)
    goto exit_spinand_move_page;
...
static rt_err_t spinand_erase_block_force(struct rt_mtd_nand_device *device,
                                          rt_uint32_t block) {
  rt_err_t result = RT_EOK;
  uint32_t page;
  ...
  result = SPINAND_FLASH_OPS->block_erase(
      SPINAND_FLASH_QSPI, (page >> 16) & 0xFF, (page >> 8) & 0xFF, page & 0xFF);
  if (result != RT_EOK)
...
static rt_err_t spinand_erase_block(struct rt_mtd_nand_device *device,
                                    rt_uint32_t block) {
  rt_err_t result = RT_EOK;
  uint32_t page;
  ...
  if (SPINAND_FLASH_OPS->block_isbad(SPINAND_FLASH_QSPI, page) != 0) {
    ...
    result =
        SPINAND_FLASH_OPS->block_erase(SPINAND_FLASH_QSPI, (page >> 16) & 0xFF,
                                       (page >> 8) & 0xFF, page & 0xFF);
    if (result != RT_EOK)
      goto exit_spinand_erase_block;
  }
...
static rt_err_t spinand_check_block(struct rt_mtd_nand_device *device,
                                    rt_uint32_t block) {
  rt_err_t result = RT_EOK;
  uint32_t page = 0;
  uint8_t isbad = 0;
...
static rt_err_t spinand_mark_badblock(struct rt_mtd_nand_device *device,
                                      rt_uint32_t block) {
  rt_err_t result = RT_EOK;
  uint32_t page = 0;
...
static struct rt_mtd_nand_driver_ops spinand_ops = {
    spinand_read_id,      spinand_read_page,   spinand_write_page,
    spinand_move_page,    spinand_erase_block, spinand_check_block,
...
rt_err_t rt_hw_mtd_spinand_init(void) {
  ...
  for (i = 0; i < MTD_SPINAND_PARTITION_NUM; i++) {
    mtd_partitions[i].page_size =
        SPINAND_FLASH_PAGE_SIZE; /* The Page size in the flash */
    mtd_partitions[i].pages_per_block =
        SPINAND_FLASH_PAGE_PER_BLOCK_NUM; /* How many page number in a block */

    mtd_partitions[i].oob_size = SPINAND_FLASH_OOB_SIZE; /* Out of bank size */
    mtd_partitions[i].oob_free =
        32; /* the free area in oob that flash driver not use */
    mtd_partitions[i].plane_num =
        SPINAND_FLASH_MCP; /* the number of plane in the NAND Flash */
...
rt_err_t rt_hw_mtd_spinand_register(const char *device_name) {
  ...
  result = rt_spi_configure(&SPINAND_FLASH_QSPI->parent,
                            &SPINAND_FLASH_QSPI->config.parent);
  RT_ASSERT(result == RT_EOK);
...
void uffs_setup_storage(struct uffs_StorageAttrSt *attr,
  ...
  rt_memcpy(attr->_uffs_data_layout, spinand_flash_data_layout,
            UFFS_SPARE_LAYOUT_SIZE);
  rt_memcpy(attr->_uffs_ecc_layout, spinand_flash_ecc_layout,
            UFFS_SPARE_LAYOUT_SIZE);
...
static int nread(int argc, char **argv) {
  ...
  if (spinand_read_page(device, page, &data_ptr[0], SPINAND_FLASH_PAGE_SIZE,
...
static int nwrite(int argc, char **argv) {
  ...
  page = page + device->block_start * device->pages_per_block;
  spinand_write_page(device, page, &data_ptr[0], SPINAND_FLASH_PAGE_SIZE, NULL,
                     0);
...
static int nmove(int argc, char **argv) {
  ...
  spinand_move_page(device, src + device->block_start * device->pages_per_block,
                    dst + device->block_start * device->pages_per_block);

  LOG_I("Move data into %d from %d in partition-index %d.", dst, src,
        partition);
...
static int nerase(int argc, char **argv) {
  ...
  LOG_I("Erased block %d in partition-index %d.", block + device->block_start,
        partition);
...
static int nerase_force(int argc, char **argv) {
  ...
  for (block = 0; block <= device->block_end; block++) {
    if (spinand_erase_block_force(device, block + device->block_start) !=
        RT_EOK)
      goto exit_nerase_force;
    LOG_I("Erased block %d in partition-index %d. forcely",
...
static rt_err_t nmarkbad(int argc, char **argv) {
  ...
  LOG_I("Marked block %d in partition-index %d.", block + device->block_start,
        partition);
...
static int ncheck_all(int argc, char **argv) {
  ...
  for (index = 0; index < device->block_total; index++) {
    LOG_I("Partion:%d Block-%d is %s", partition, index,
          spinand_check_block(device, index) ? "bad" : "good");
  }
