...
const struct nu_spinand_info g_spinandflash_list[] = {
    ...
    {/* Here, we just only define 64B for spare area, not ECC area. */
     ...
     64,
     0,
     "Winbond 512MB: 2048+128@64@4096",
#if defined(RT_USING_DFS_UFFS)
     {/* For storing Seal-byte at 0x39. Need 15-Bytes */
...
#define SPINAND_LIST_ELEMENT_NUM                                               \
  (sizeof(g_spinandflash_list) / sizeof(struct nu_spinand_info))
...
static rt_err_t spinand_die_select(struct rt_qspi_device *qspi,
                                   uint8_t select_die) {
...
#define BUSY_CKECKING_TIMEOUT_MS 3000
  ...
  uint32_t u32CheckingDuration =
      rt_tick_from_millisecond(BUSY_CKECKING_TIMEOUT_MS);
  uint32_t u32Start = rt_tick_get();

  do {
    result = nu_qspi_send_then_recv(qspi, &au8Cmd[0], sizeof(au8Cmd),
                                    (void *)&SR, 1);
    if (result != RT_EOK)
...
static rt_err_t spinand_program_dataload(struct rt_qspi_device *qspi,
                                         uint8_t u8AddrH, uint8_t u8AddrL,
                                         uint8_t *pu8DataBuff,
  ...
  result = nu_qspi_transfer_message(
      qspi, (struct rt_qspi_message *)&qspi_messages[0]);
...
static uint8_t spinand_status_register_read(struct rt_qspi_device *qspi,
                                            uint8_t u8SRSel) {
  uint8_t u8SR = 0;
  uint8_t au8Cmd[2];
  ...
  if (nu_qspi_send_then_recv(qspi, &au8Cmd[0], sizeof(au8Cmd), &u8SR, 1) !=
      RT_EOK)
    RT_ASSERT(0);
...
static rt_err_t spinand_status_register_write(struct rt_qspi_device *qspi,
                                              uint8_t u8SRSel,
                                              uint8_t u8Value) {
  rt_err_t result = RT_EOK;
  uint8_t au8Cmd[3];
...
static rt_err_t spinand_program_execute(struct rt_qspi_device *qspi,
                                        uint8_t u8Addr2, uint8_t u8Addr1,
                                        uint8_t u8Addr0) {
  rt_err_t result;
...
static rt_err_t spinand_normal_read(struct rt_qspi_device *qspi,
                                    uint8_t u8AddrH, uint8_t u8AddrL,
                                    uint8_t *pu8Buff, uint32_t u32Count) {
  uint8_t au8Cmd[4];
  ...
  return nu_qspi_send_then_recv(qspi, &au8Cmd[0], sizeof(au8Cmd), pu8Buff,
                                u32Count);
}

static rt_err_t spinand_protect_set(struct rt_qspi_device *qspi,
                                    uint8_t u8Protect) {
...
static rt_err_t spinand_hwecc_set(struct rt_qspi_device *qspi,
                                  uint8_t u8Enable) {
  uint8_t u8SR =
      spinand_status_register_read(qspi, 2); // Read status register 2
...
static rt_err_t spinand_read_dataload(struct rt_qspi_device *qspi,
                                      uint8_t u8Addr2, uint8_t u8Addr1,
                                      uint8_t u8Addr0) {
  rt_err_t result = RT_EOK;
  uint8_t au8Cmd[4];
...
static uint8_t spinand_block_isbad(struct rt_qspi_device *qspi,
...
again_spinand_block_isbad:
  ...
  result = spinand_read_dataload(
      qspi, (u32PageAddr >> 16) & 0xFF, (u32PageAddr >> 8) & 0xFF,
      u32PageAddr & 0xFF); // Read the first page of a block
  RT_ASSERT(result == RT_EOK);

  result =
      spinand_normal_read(qspi, (SPINAND_FLASH_PAGE_SIZE >> 8) & 0xff,
                          SPINAND_FLASH_PAGE_SIZE & 0xff, &read_buf,
                          1); // Read bad block mark at 0x800 update at v.1.0.8
  RT_ASSERT(result == RT_EOK);
  ...
  if (((u32PageAddr %
        (SPINAND_FLASH_PAGE_PER_BLOCK_NUM * SPINAND_FLASH_PAGE_SIZE)) == 0)) {
...
static rt_err_t spinand_buffermode_set(struct rt_qspi_device *qspi,
                                       uint8_t u8Enable) {
  uint8_t u8SR =
      spinand_status_register_read(qspi, 2); // Read status register 2
...
static rt_err_t spinand_block_erase(struct rt_qspi_device *qspi,
                                    uint8_t u8Addr2, uint8_t u8Addr1,
                                    uint8_t u8Addr0) {
  rt_err_t result;
  uint8_t u8WECmd = 0x06;
  ...
  if ((result = nu_qspi_send(qspi, &au8EraseCmd[0], sizeof(au8EraseCmd))) !=
      RT_EOK)
    goto exit_spinand_block_erase;
...
static rt_err_t spinand_block_markbad(struct rt_qspi_device *qspi,
                                      uint32_t u32PageAddr) {
  rt_err_t result = RT_EOK;

  uint8_t u8BadBlockMarker = 0xF0;

  result = spinand_block_erase(qspi, (u32PageAddr >> 16) & 0xFF,
                               (u32PageAddr >> 8) & 0xFF, u32PageAddr & 0xFF);
  if (result != RT_EOK)
    return result;

  result = spinand_program_dataload(qspi, (SPINAND_FLASH_PAGE_SIZE >> 8) & 0xff,
                                    SPINAND_FLASH_PAGE_SIZE & 0xff,
                                    &u8BadBlockMarker, 1, 0, 0);
  if (result != RT_EOK)
    return result;

  return spinand_program_execute(qspi, (u32PageAddr >> 16) & 0xFF,
                                 (u32PageAddr >> 8) & 0xFF, u32PageAddr & 0xFF);
}

static rt_err_t spinand_read_quadoutput(struct rt_qspi_device *qspi,
                                        uint8_t u8AddrH, uint8_t u8AddrL,
                                        uint8_t *pu8DataBuff,
                                        uint32_t u32DataCount) {
  ...
  return nu_qspi_transfer_message(qspi,
                                  (struct rt_qspi_message *)&qspi_messages);
}
...
int spinand_supported_flash_size(void) { return SPINAND_LIST_ELEMENT_NUM; }
...
static rt_err_t spinand_info_read(struct rt_qspi_device *qspi) {
  ...
  for (i = 0; i < SPINAND_LIST_ELEMENT_NUM; i++) {
    ...
    {
      rt_memcpy((void *)&spinand_flash_data_layout[0],
                (void *)&g_spinandflash_list[i].au8DataLayout[0],
                SPINAND_SPARE_LAYOUT_SIZE);
      rt_memcpy((void *)&spinand_flash_ecc_layout[0],
                (void *)&g_spinandflash_list[i].au8EccLayout[0],
                SPINAND_SPARE_LAYOUT_SIZE);

      rt_memcpy(SPINAND_FLASH_INFO, &g_spinandflash_list[i],
                sizeof(struct nu_spinand_info));
