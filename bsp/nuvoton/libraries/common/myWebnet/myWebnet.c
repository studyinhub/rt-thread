...
char init_wb_dirs[NUMBER_OF_STRING][MAX_STRING_SIZE] = {"/css", "/js",
                                                        "/fonts"};
...
int init_tftps(char *root_path) {
  log_d("启动 tftp server:%s", root_path);
  ...
  g_tftp_tid =
      rt_thread_create("tftps", tftp_server_thread, tftp_server, 2048, 18, 20);
  if (g_tftp_tid == NULL) {
    log_e("create tftps thread faild");
  }
  if (rt_thread_startup(g_tftp_tid) == RT_EOK) {
    log_d("tftps Thread 启动成功");
  }
...
void asp_var_version(struct webnet_session *session) {
  ...
  static const char *version =
      "<html><body><font size=\"+2\">RT-Thread %d.%d.%d %s</font><br><br>"
      "<a href=\"javascript:history.go(-1);\">Go back to "
      "root</a></html></body>";

  webnet_session_printf(session, version, RT_VERSION, RT_SUBVERSION,
                        RT_REVISION, buildtime);
}
...
void cgi_calc_handler(struct webnet_session *session) {
  ...
  static const char *body =
      "<body><form method=\"post\" action=\"/cgi-bin/calc\"><input "
      "type=\"text\" name=\"a\" value=\"%d\"> "
      "+ <input type=\"text\" name=\"b\" value=\"%d\"> = %d<br><input "
      "type=\"submit\" value=\"\xBC\xC6\xCB\xE3\"></form>"
...
void cgi_getconfig_handler(struct webnet_session *session) {
  ...
  // cJSON_AddItemToObject(g_root, "webnet_in_ram",
  // cJSON_CreateBool(webnet_in_ram)); cJSON_AddItemToObject(g_root, "web_root",
  // cJSON_CreateString(WEB_ROOT));
  rt_sprintf(g_BUF_CONFIG_JSON, "%s", cJSON_Print(g_root));

  /* set http header */
  session->request->result_code = 200;
  webnet_session_set_header(session, mime_get_type("json"), 200, "Ok",
                            strlen(g_BUF_CONFIG_JSON));
  webnet_session_write(session, (const rt_uint8_t *)g_BUF_CONFIG_JSON,
                       rt_strlen(g_BUF_CONFIG_JSON));
}
...
void cgi_putconfig_handler(struct webnet_session *session) {
  ...
  // print_time();
  // cJSON_ReplaceItemInObject(g_root, "lastConfigAt",
  // cJSON_CreateString(g_FmtTimeStr));
  ...
  // printJSON(pPort1);
  // cJSON_ReplaceItemInObject(pPort1, "baud_rate",
  // cJSON_CreateNumber(atoi(port1_baudrate))); printJSON(pPort1);
  // printJSON(g_root);
