...
/* 在空闲线程的回调函数里喂狗 */
static void idle_hook(void) { feed_wdt(0); }
...
int enable_wdt() {
  ...
  // log_d("Don't forget feed the dog in %d seconds!", timeout);
...
static int wdt_sample(int argc, char *argv[]) {
  ...
  log_d("Watchdog is ON!Don't forget feed the dog in %d seconds peroidly!",
        timeout);

  /* 设置空闲线程回调函数 */
  // rt_thread_idle_sethook(idle_hook);

  rt_timer_init(&timer1, "tm1", cb_timer1, RT_NULL, timer1_timeout_seconds,
