#!/usr/bin/env python
"""
async_twisted_client_serial_ascii
...
# ---------------------------------------------------------------------------#
# https://wiki.jikexueyuan.com/project/twisted-intro/p02.html
# SERIAL_PORT = "/tmp/ptyp0"
SERIAL_PORT = "/dev/tty.wchusbserial14220"
# 读取 holdings 寄存器，从 1 开始读取 2 个字节
STATUS_REGS = (1, 2)
STATUS_COILS = (1, 2)
CLIENT_DELAY = 10
# 从机地址
...
class ExampleProtocol(ModbusClientProtocol):
    ...
    def __init__(self, framer):
        ...
        log.debug("开始循环发送，间隔：%d",CLIENT_DELAY)
        reactor.callLater(0, self.fetch_holding_registers)
    ...
    def fetch_holding_registers(self):
        ...
        # d = self.readwrite_registers(read_address=4,read_count=3,write_address=11,write_registers=(0x009B,0x0001),unit=UNIT)
        # :010300000007F5
        # d=self.read_holding_registers(address=0,count=7,unit=UNIT)

        # :0106000C0001EC
        # d=self.write_registers(address=)
