...
// LRC checks the content of the message other than [:] of START and [CR][LF] of
// END. The sending side calculates and sets. The receiving side calculates
...
// received LRC do not match. Add up the byte number of the message consisting
// of 8 consecutive bits. The result except the carry (overflow) is converted to
// 2’s complement. http://www.ip33.com/lrc.html 如果内容本身是已经 ASCII
// 转换过后的 buf 计算从 ： 之后的所有数据
...
int ASCII_LRC(uint8_t *buf, uint16_t len) {
  ...
  uint16_t hLen =
      len /
      2; // len：整个 frame -1(:) - 4(lrc),注意 lrc 一个字节，但是是两个字符
...
uint8_t calculate_lrc(const uint8_t *data, uint16_t len) {
  uint8_t lrc = 0;
...
void uint8_to_ascii(uint8_t value, uint8_t *out) {
  // 高 4 位
  ...
  // 转成 0~9 A~F
  out[0] = hi <= 9 ? '0' + hi : 'A' + hi - 10;
  out[1] = lo <= 9 ? '0' + lo : 'A' + lo - 10;
}

uint8_t ascii_to_uint8(const uint8_t *hex) {
  uint8_t result = 0;
