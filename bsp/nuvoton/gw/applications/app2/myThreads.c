  ...
  while (1) {
    ...
    if (rtu_send_mq.entry) {
      ...
      if (rtu_send_mq.entry > 5)
        log_d("rtu_send_mq(%d/%d): %d %d", rtu_send_mq.entry,
              rtu_send_mq.max_msgs, rtu_write.start_addr,
              rtu_write.wrRegQuantity);
...
static void proc_response(struct SER_MSG *ser_msg) {
  ...
  if (ret != 1 && ret != 2) {
    ...
    ulog_hexdump("error data:", 16, ser_msg->req_data,
                 rt_strlen(ser_msg->req_data));
...
static void thread_asc_entry(void *parameter) {
  ...
  while (1) {
    ...
    rt_err_t ret;
    ret = rt_mq_recv(&asc_recv_mq, (rt_ubase_t *)&ser_msg, sizeof ser_msg,
                     RT_WAITING_FOREVER);
    ...
    // log_d("asc_recv_mq.entry:%d/%d size:%d",
    // asc_recv_mq.entry,asc_recv_mq.max_msgs, asc_recv_mq.msg_size);
...
int threads_init(void) {
  ...
  */
  ...
  if (result != RT_EOK) {
    log_e("init rtu_send_mq queue failed.\n");
    return -1;
  }
...
rt_int16_t strip_raw(char *buf) {
  ...
  // 确保只有一个 CRLF 结尾
  if (frame_len > 2 && last_crlf > valid_colon + 2 &&
      memcmp(last_crlf - 2, "\r\n", 2) == 0) {
