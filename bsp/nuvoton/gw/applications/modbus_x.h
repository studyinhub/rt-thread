...
extern int rs485_receive(struct SER_PORT* port, uint8_t *buf, int bufsz, int timeout);

extern rt_err_t ascii_parse_request(struct SER_MSG* ser_msg);
...
extern rt_err_t modbus_read_regs(agile_modbus_t *ctx,uint16_t rdHead,uint16_t rdQuantity,uint16_t *buf);
extern rt_err_t modbus_write_regs(agile_modbus_t *ctx,uint16_t wrHead,uint16_t wrRegQuantity,uint16_t *buf);
