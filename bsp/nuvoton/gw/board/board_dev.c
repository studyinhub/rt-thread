...
// nand1:
//
// uboot:0x100000 8 - 15 block ;1MB; 325k ~ 2.53 block, 325- 128*2 =
// 69k,  34.5page mtd_nand read nand0 10 43 app:  0x200000 16-23 block; ;1MB;
// fs:   0x300000 24
// 24 个 block 是 3072k  3MB
// nand0 3MB 基本上存储的都是系统的 spl uboot app env，这些，所以 nand0
// 不要存入用户数据，这部分占用的是 3MB 之前的数据，给 app 留了 1MB 的空间 nand1
// 的空间是 128-3 = 125MB， 起始地址是 0x300000
// ,如果在这个位置烧写用户数据会怎么样，那么用到的文件系统是 uffs，如何生成 uffs
// 的 image 呢？ nand2 是整个分区，128MB， nand1
...
static int rt_hw_spinand_init(void) {
  if (nu_qspi_bus_attach_device("qspi0", "qspi01", 4, RT_NULL, RT_NULL) !=
      RT_EOK)
    return -1;
...
int test_uart6(int argc, char **argv) {
  ...
  struct serial_configure config =
      RT_SERIAL_CONFIG_DEFAULT; /* 初始化配置参数 */
