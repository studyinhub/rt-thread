 ...
 * routine. 2021-02-28     Meco Man     add RT_KSERVICE_USING_STDLIB
 */
...
RT_WEAK rt_int32_t rt_memcmp(const void *cs, const void *ct, rt_ubase_t count) {
  ...
  for (su1 = (const unsigned char *)cs, su2 = (const unsigned char *)ct;
...
#ifdef RT_PRINTF_PRECISION
      str = print_number(str, end, (long)va_arg(args, void *), 16, field_width,
                         precision, flags);
#else
      str = print_number(str, end, (long)va_arg(args, void *), 16, field_width,
                         flags);
 ...
 */
rt_device_t rt_console_get_device(void) { return _console_device; }
