 ...
 * 2013-07-05     Bernard      Remove initialization feature for MS VC++
 * compiler 2015-02-06     Bernard      Remove the MS VC++ support and move to
 * the kernel 2015-05-04     Bernard      Rename it to components.c because
 * compiling issue in some IDEs. 2015-07-29     Arda.Fu      Add support to use
 * RT_USING_USER_MAIN with IAR 2018-11-22     Jesven       Add secondary cpu
 * boot up
 */
...
/*
 * Components Initialization will initialize some driver and components as
 * following order: rti_start         --> 0 BOARD_EXPORT      --> 1
 * rti_board_end     --> 1.end
 *
 * DEVICE_EXPORT     --> 2
 * COMPONENT_EXPORT  --> 3
 ...
 * rti_end           --> 6.end
 *
 * These automatically initialization, the driver or component initial function
 * must be defined with: INIT_BOARD_EXPORT(fn); INIT_DEVICE_EXPORT(fn);
 * ...
 * INIT_APP_EXPORT(fn);
 * etc.
 */
